...
df_orders_transform_data = df_orders.withColumn(
    "status",
...
check_null_columns = df_orders_transform_data.columns
find_null_values = " OR ".join([f'{col} IS NULL' for col in check_null_columns])
df_null_fields = df_orders_transform_data.filter(find_null_values)
